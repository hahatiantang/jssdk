...
// 当前页
            ...
            data.clBookData = clBookData;
            data.podData = clBookData.content_list;
            // 总页数
            that.allPageCount = 0;
            /*data.width = clBookData.book_width - 17 * data.ratioWidth;
             data.height = clBookData.book_height -8.5 * data.ratioHeight;*/
  ...
  FB.prototype.bookTemplate = function (n) {
    ...
    var pageCenter = $('<div></div>')
    var pageBox_copy = $('<div class="page_copy"></div>')
    ...
    if(podData[n].element_list.length>0){
      for(var j=0; j<podData[n].element_list.length;j++){
        ...
        pageList.appendTo(pageCenter)


      }
    }
    var clBookData = data.clBookData;
    pageCenter.css({
      'position':'relative'
    })
    var pageZoom = podData[n].page_zoom;
    if(pageZoom == 0){
      pageCenter.css({
        'left':clBookData.content_padding_left + 'px',
        'top':clBookData.content_padding_top + 'px',
        'width':clBookData.content_width + 'px',
        'height':clBookData.content_height + 'px'
      })
    }else if(pageZoom == 1){
      pageCenter.css({
        'left':0 + 'px',
      ...
      pageCenter.css({
        'left':clBookData.content_padding_left * pageZoom + 'px',
        'top':clBookData.content_padding_top * pageZoom + 'px',
        'width':clBookData.content_width * pageZoom + 'px',
        'height':clBookData.content_height * pageZoom + 'px'
      })
    }

    pageCenter.appendTo(pageBox)
    ...
    if(podData[n].page_background_mode == 0){
      pageBox.css({
        'background-size':'100%'
      })
    }else{
      pageBox_copy.css({
        ...
        'position':'absolute',
        'transform': 'scale(.5)',
        'left':-(data.width*0.5)+'px',
        'top':-(data.height* 0.5)+'px'
      })
      pageBox_copy.appendTo(pageBox)
    }
    return  pageBox
  };
  /*初始化渲染*/
